...
void main() {
  ...
  float xShake = snoise(vec2((progress * 0.1 / speed), position.x));
  float yShake = snoise(vec2((progress * 0.1 / speed), position.y));
  float zShake = snoise(vec2((progress * 0.1 / speed), position.z));
  vec3 shake = vec3(xShake, yShake, zShake);
  // when near time is 0 dont shake
  // multiply by progress gives you a boiling effect, perfect for a global temperture data
  // vec3 newPosition = position + shake * shakingStrength * progress;
  vec3 newPosition = position + shake * shakingStrength * nearTime;
